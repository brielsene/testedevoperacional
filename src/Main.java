...
public class Main {
		...
		if (usuariosSearch.size() > 0) {
			...
			if ((usuarioLogado.getSenha().equals(senha))) {
				...
				if (usuarioLogado.IsAdmin()) {
					System.out.println("Escolha uma empresa: ");
					...
					for (Empresa empresa : empresas) {
						if (empresa.getId().equals(escolha)) {
					...
					while (usuarioLogado.getEmpresa() != null) {

						System.out.println("Digite uma opção: ");
						System.out.println("1 - Listar vendas");
						System.out.println("2 - Ver produtos");
						System.out.println("0 - Sair da empresa");
							...
							case 1: {
								...
								vendas.stream().forEach(venda -> {
									if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
										...
										System.out.println("Total Venda: R$" + df.format((venda.getValor())));
										System.out.println(
												"Total Taxa a ser paga: R$" + df.format(venda.getComissaoSistema()));

										System.out.println("Total Líquido  para empresa:" + "    R$"
												+ df.format((venda.getValor())));
										System.out.println(
												"************************************************************");
									}
					...
					case 1: {
						...
						vendas.stream().forEach(venda -> {
							if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
								...
								System.out.println("Total Venda: R$" + df.format((venda.getValor())));
								System.out.println("Total Taxa a ser paga: R$" + df.format(venda.getComissaoSistema()));

								System.out.println(
										"Total Líquido  para empresa:" + "    R$" + df.format((venda.getValor())));
								System.out.println("************************************************************");
							}

						});
						System.out.println("Saldo Empresa: R$" + df.format(usuarioLogado.getEmpresa().getSaldo()));
					...
					case 1: {
						...
						do {
							...
							for (Produto produtoSearch : produtos) {
								if (produtoSearch.getId().equals(escolhaProduto)
									...
									System.out.println(produtoSearch.getNome() + ", adicionado com sucesso");
									System.out.println();
								}
						...
						carrinho.stream().forEach(x -> {
							if (x.getEmpresa().getId().equals(escolhaEmpresa)) {
								...
								// Ajuste na inserção da taxa da empresa
								System.out.println(x.getId() + " - " + x.getNome() + "    R$"
										+ (x.getPreco() + ", taxa da empresa: R$"
												+ df.format((x.getPreco() * (x.getEmpresa().getTaxa())))));
							}
						});
						...
						// valor total acom adição da taxa
						// formatação para casas decimais
						venda.setValor(venda.getValor() + (venda.getValor() * empresaEscolhida.getTaxa()));
						System.out.println("Total: R$" + df.format(venda.getValor()));
						System.out.println("************************************************************");
						carrinho.clear();
						executar(usuarios, clientes, empresas, produtos, carrinho, vendas);
					}
					case 2: {
						...
						vendas.stream().forEach(venda -> {
							if (venda.getCliente().getUsername().equals(usuarioLogado.getUsername())) {
								...
								venda.getItens().stream().forEach(x -> {
									// Adicionando informação de taxa da empresa para o cliente saber
									System.out.println(x.getId() + " - " + x.getNome() + "    R$" + x.getPreco()
											+ ", Taxa da empresa: "
											+ df.format((x.getPreco() * x.getEmpresa().getTaxa())));
								});
	...
	public static Venda criarVenda(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
		...
		// Ajuste do saldo total da empresa, com taxa e comissão de sistema
		empresa.setSaldo(empresa.getSaldo() + venda.getValor() + (venda.getValor() * empresa.getTaxa()));
